...
function App() {
  ...
  const handleDelete = (id) => {
    ...
    // If the deleted task is being edited, reset edit state
    if (editid === id) {
      setEditid(0);
      setTask("");
    }
  ...
  return (
    <div className={"App " + theme}>
      <div className="container">
        <Header setTheme={setTheme} theme={theme} >Tododify</Header>
        <main>
          <AddTask className="AT" handleSubmit={handleSubmit} editid={editid} task={task} setTask={setTask}/>
          <ShowTask className="ST" tasklist={tasklist} setTasklist={setTasklist} handleEdit={handleEdit} handleDelete={handleDelete}/>
        </main>
        <Footer />
      </div>
    </div>
